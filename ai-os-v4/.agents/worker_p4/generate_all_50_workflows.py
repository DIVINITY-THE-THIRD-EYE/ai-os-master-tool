...
wf_list = [
    # 1
    ("software_development_workflow.md", "Software Development Workflow",
    ...
    # 2
    ("website_creation_workflow.md", "Website Creation Workflow",
    ...
    # 3
    ("flutter_app_development_workflow.md", "Flutter App Development Workflow",
    ...
    # 4
    ("react_app_development_workflow.md", "React App Development Workflow",
    ...
    # 5
    ("node_backend_development_workflow.md", "Node Backend Development Workflow",
    ...
    # 6
    ("api_development_workflow.md", "API Development Workflow",
     ...
     [("API Specification & Contract Design", "Endpoint functional requirements.", "Draft OpenAPI 3.1 YAML spec including paths, query parameters, request bodies, response codes, and schemas.", "Validated OpenAPI YAML specification document.", "OpenAPI linter (Spectral) passes with zero errors."),
      ("Mock Server & SDK Generation", "OpenAPI spec file.", "Spin up Prism mock server for frontend parallel development and auto-generate client SDKs via OpenAPI Generator.", "Live mock server URL and generated SDK packages.", "Frontend developer contract confirmation on mock responses."),
      ("Endpoint Logic Implementation", "OpenAPI spec, database connections, service controllers.", "Develop controller handlers matching spec paths, bind DTO validations, implement business logic.", "Executable API route code co-located with unit tests.", "Unit tests covering 200 OK, 400 Bad Request, 401 Unauthorized, and 500 Internal Error codes."),
      ("Contract Testing & Security Validation", "Live service build, Prism / Pact test harness, OWASP ZAP scanner.", "Execute Pact contract tests verifying request/response payload adherence; run OWASP ZAP API scan.", "Contract verification report and security audit findings.", "100% Pact contract alignment and zero high-severity security findings."),
      ("API Gateway Routing & Versioning", "Verified service build, API Gateway configuration (Kong/Apigee/AWS API GW).", "Configure gateway route matching, CORS policy, rate limiting policies, and TLS termination.", "Deployed API gateway configuration.", "HTTP 200 response verification from external gateway endpoint URL.")],
    ...
    # 7
    ("ai_research_workflow.md", "AI Research Workflow",
     ...
     [("Literature Review & State of the Art (SOTA) Analysis", "Research topic query, academic databases (ArXiv, PapersWithCode).", "Aggregate SOTA publications, compare architectural approaches, identify research gaps.", "Literature Review & Benchmark Survey synthesis document.", "AI Research Lead approval of research hypothesis."),
      ("Experimental Setup & Dataset Curation", "Raw datasets, compute environment (PyTorch, CUDA, WandB).", "Clean raw data, create reproducible train/val/test splits, verify label distribution, configure dataset loaders.", "Standardized dataset splits and data pipeline scripts.", "Dataset validation check confirming no data leakage across splits."),
      ("Model Architecture Prototyping", "Dataset loaders, model design hypotheses.", "Write modular PyTorch/JAX model components, construct loss functions, write training loops with mixed-precision support.", "Model codebase and modular component unit tests.", "Forward-pass sanity check with dummy input tensor passing without dimension mismatches."),
      ("Experimental Execution & Hyperparameter Tracking", "Model codebase, WandB / MLflow tracking, GPU cluster.", "Execute training sweeps across hyperparameter grids; track loss curves, accuracy, latency, and memory utilization.", "Experiment logs, checkpoint weights, and WandB runs dashboard.", "Validation metric convergence without gradient explosion or vanishing."),
      ("Results Synthesis & Paper Drafting", "WandB experiment runs, evaluation metrics, visual charts.", "Compile ablation study tables, draft methodology section, write comparative evaluation vs SOTA baselines.", "Comprehensive Research Paper draft (LaTeX format) and model weights repository.", "Internal peer review approval from AI Research Lead.")],
    ...
    # 8
    ("mechanical_design_workflow.md", "Mechanical Design Workflow",
     ...
     [("3D Solid Modeling & Parameterization", "Product requirements document, envelope dimensions, interference parameters.", "Create 3D parametric CAD parts, assemble component trees, define mate relationships, apply material properties.", "Parametric CAD model files (.SLDPRT, .SLDASM / STEP).", "Interference and clearance check passing with 0 overlapping volumes."),
      ("Structural & Stress Analysis (FEA)", "3D assembly model, load boundary conditions, yield strength constraints.", "Apply mesh constraints, define load cases (static force, dynamic vibration, thermal stress), run FEA solver.", "FEA simulation report detailing Von Mises stress distribution and Factor of Safety (FoS).", "Minimum FoS >= 2.0 across all critical load cases."),
      ("GD&T Drawing & Tolerance Stack-Up", "3D assembly model, FEA verification.", "Generate 2D drafting drawings, apply GD&T datums and tolerances (ISO 2768 / ASME Y14.5), perform tolerance stack-up analysis.", "Dimensioned 2D engineering drawings (PDF / DWG).", "Tolerance stack-up verification showing zero assembly binding at MMC (Maximum Material Condition)."),
      ("Design for Manufacturability (DFM) Review", "CAD models, 2D drawings, vendor tooling limits.", "Review draft angles, wall thicknesses, bend radii, and machining access with manufacturing suppliers.", "DFM review report and updated CAD model.", "Supplier DFM sign-off with 0 unmanufacturable features."),
      ("BOM Generation & Prototype Release", "Final CAD package, verified drawing set.", "Generate Bill of Materials (BOM) with part numbers, quantities, material callouts, and vendor sources; release to ERP.", "Released Engineering BOM (eBOM) and prototype procurement package.", "Engineering Change Order (ECO) approval from Mechanical Engineer.")],
    ...
    # 9
    ("manufacturing_process_workflow.md", "Manufacturing Process Workflow",
     ...
     [("Process Routing & Operation Sheet Definition", "Released eBOM, engineering drawings.", "Define step-by-step manufacturing routing, select machine centers (CNC, lathe, injection molding), calculate cycle times.", "Manufacturing Process Plan (MPP) and Operation Sheets.", "Process Engineer approval of operational routing sequence."),
      ("Tooling Design & CNC Programming", "CAD models, MPP operation specs, CAM software (Mastercam/Siemens NX).", "Design custom fixtures and jigs, generate CNC toolpaths, run CAM simulation to detect collisions.", "CAM files, G-code programs, and physical tooling fixtures.", "CAM simulation verification with 0 tool collision events."),
      ("Pilot Run & First Article Inspection (FAI)", "Tooling fixtures, CNC G-code, raw material stock.", "Execute pilot production run of 10 units, inspect dimensions using CMM (Coordinate Measuring Machine).", "First Article Inspection Report (FAIR) according to AS9102 standard.", "100% dimensional compliance on all critical-to-quality (CTQ) drawing features."),
      ("Statistical Process Control (SPC) Setup", "CMM measurement data, production line sensors.", "Establish control charts (X-bar, R-charts), define control limits, measure process capability index (Cpk).", "SPC dashboard and process capability report.", "Cpk index >= 1.33 across all CTQ parameters."),
      ("Mass Production Sign-Off", "Passed FAIR, SPC capability report, standard work instructions.", "Publish standard operating procedures (SOPs) on shop floor terminals; authorize full-scale production run.", "Production Authorization Certificate and live MES routing.", "Quality Control Auditor formal sign-off.")],
    ...
    # 10
    ("construction_project_workflow.md", "Construction Project Workflow",
     ...
     [("Blueprint Audit & BIM Coordination", "Architectural/structural CAD/BIM models (Revit), municipal permits.", "Run Building Information Modeling (BIM) clash detection between structural, MEP (mechanical, electrical, plumbing) systems.", "Clash Detection Report and coordinated BIM master model.", "BIM Coordinator sign-off with 0 hard structural clashes."),
      ("Site Preparation & Excavation", "Site survey drawings, geotechnical soil report, heavy equipment schedule.", "Execute site grading, soil compaction testing, utility line layout, and foundation excavation.", "Soil compaction test results and survey verification log.", "Geotechnical engineer validation of soil bearing capacity >= target kPa."),
      ("Structural Framing & Concrete Pour", "Structural drawings, rebar schedules, concrete batch mix specs.", "Form foundation footings, place rebar cages, conduct slump test, perform concrete pour, monitor curing strength.", "Concrete cylinder break test reports (7-day and 28-day).", "28-day break test confirming compressive strength meets specified PSI/MPa."),
      ("MEP Installation & Enclosure", "MEP drawings, framing inspection sign-off.", "Install framing studs, exterior cladding, roofing, electrical wiring, plumbing runs, and HVAC ductwork.", "Rough-in inspection report from municipal inspector.", "Passed municipal rough-in inspections for electrical, plumbing, and framing."),
      ("Commissioning & Handover", "As-built drawings, punch list, HVAC balancing reports.", "Perform HVAC balancing, fire alarm system testing, resolve punch list items, conduct final walkthrough.", "Certificate of Occupancy (CO) and final handover binder.", "Issuance of official Certificate of Occupancy by municipal authority.")],
...
print(f"Batch 1 (1-10) created successfully.")
