...
print_and_log("   - Required minimum: 450 files")
...
print_and_log("   - Phase 00 Foundation:")
print_and_log(f"     * File Count: {p0_count} (min required: 20) -> {'PASS' if p0_count >= 20 else 'FAIL'}")
print_and_log(
    f"     * CONVENTIONS.md present & defines naming, directory, metadata, file format standards: {'PASS' if conv_valid else 'FAIL'}"
)

# Phase 01 Core Runtime
p1_count = phase_file_counts.get("phase_01_core_runtime", 0)
print_and_log("   - Phase 01 Core Runtime:")
...
print_and_log("   - Phase 02 Agent Framework:")
print_and_log(
    f"     * Spec Files Count: {len(spec_files)} (required: 35) -> {'PASS' if len(spec_files) == 35 else 'FAIL'}"
)
print_and_log(
    f"     * Prompt Files Count: {len(prompt_files)} (required: 35) -> {'PASS' if len(prompt_files) == 35 else 'FAIL'}"
)
print_and_log(
    f"     * Total P2 Files: {len(spec_files) + len(prompt_files)} (required: 70) -> {'PASS' if p2_count_pass else 'FAIL'}"
)
...
p3_prompt_files = (
    [os.path.join(r, f) for r, d, filenames in os.walk(p3_dir) for f in filenames if f.endswith(".md")]
...
print_and_log("   - Phase 03 Prompt Library:")
print_and_log(
    f"     * Total Prompt Files: {len(p3_prompt_files)} (min required: 120) -> {'PASS' if len(p3_prompt_files) >= 120 else 'FAIL'}"
)
print_and_log(
    f"     * Domain Subdirectories Count: {len(p3_domains)} (required: 20) -> {'PASS' if len(p3_domains) == 20 else 'FAIL'}"
)
print_and_log(
    f"     * Word Count Compliance (>= 200 words each): {'PASS' if words_pass else 'FAIL'} (Min: {min_words}, Max: {max_words}, Avg: {avg_words:.1f} words)"
)

# Phase 04 Workflow Library
p4_count = phase_file_counts.get("phase_04_workflow_library", 0)
print_and_log("   - Phase 04 Workflow Library:")
...
print_and_log("   - Phase 05 Knowledge Platform:")
...
print_and_log("   - Phase 06 Memory System:")
...
print_and_log("   - Phase 07 Decision Engine:")
...
print_and_log("   - Phase 08 Reflection & Learning:")
...
print_and_log("   - Phase 09 Verification Platform:")
...
print_and_log("   - Phase 10 Template Library:")
...
p11_schema_files = (
    [os.path.join(r, f) for r, d, filenames in os.walk(p11_dir) for f in filenames if f.endswith(".json")]
    if os.path.exists(p11_dir)
    else []
)
...
print_and_log("   - Phase 11 Schemas:")
print_and_log(
    f"     * JSON Schema File Count: {len(p11_schema_files)} (min required: 40) -> {'PASS' if len(p11_schema_files) >= 40 else 'FAIL'}"
)
print_and_log(f"     * JSON Syntax Validity: {'PASS' if json_valid_all else 'FAIL'}")
print_and_log(
    f"     * Presence of ($schema, title, type, properties): {'PASS' if required_schema_fields_all else 'FAIL'}"
)

# Phase 12 Domain Skill Packs
p12_dir = os.path.join(repo_root, "phase_12_domain_skill_packs")
p12_domains = (
    [d for d in os.listdir(p12_dir) if os.path.isdir(os.path.join(p12_dir, d))] if os.path.exists(p12_dir) else []
)
...
print_and_log("   - Phase 12 Domain Skill Packs:")
print_and_log(
    f"     * Domain Subdirectories Count: {len(p12_domains)} (required EXACTLY: 18) -> {'PASS' if len(p12_domains) == 18 else 'FAIL'}"
)
print_and_log(
    f"     * All 18 Domains contain >= 7 of 8 required subdirectories: {'PASS' if p12_subdirs_pass else 'FAIL'}"
)

# Phase 13 Plugin Framework
p13_count = phase_file_counts.get("phase_13_plugin_framework", 0)
print_and_log("   - Phase 13 Plugin Framework:")
...
print_and_log("   - Phase 14 Runtime Policies:")
...
print_and_log("   - Phase 15 Enterprise Documentation:")
...
print_and_log(
    f"   - Zero Empty (0-byte) Files: {'PASS' if len(zero_byte_files) == 0 else 'FAIL'} (Count: {len(zero_byte_files)})"
)
...
print_and_log(
    f"   - Zero Placeholder / Stub Files: {'PASS' if len(stub_files) == 0 else 'FAIL'} (Count: {len(stub_files)})"
)
